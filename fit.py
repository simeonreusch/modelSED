...
class FitSpectrum():
    ...
    def fit_blackbody(self, **kwargs):
        ...
        minimizer = Minimizer(self._bb_minimizer, params, fcn_args=(x, data), fcn_kws={"extinction_av": extinction_av, "extinction_rv": extinction_rv})
        result = minimizer.minimize()
        parameters = result.params.valuesdict()

        spectrum = blackbody_spectrum(temperature=parameters["temp"], scale=parameters["scale"], extinction_av=extinction_av, extinction_rv=extinction_rv, redshift=self.redshift)
        spectrum_evaluated = self._evaluate_spectrum(spectrum, self.magnitudes)
    ...
    def _evaluate_spectrum(spectrum, magnitudes: dict):
        ...
        for key in magnitudes.keys():
            if key != "mjd":
                ...
                temp = {key: {"observed": mag, "observed_err": mag_err, "model": mag_model, "wavelength": filter_wl[key], "frequency": const.c.value/(filter_wl[key]*1E-10)}}
    ...
    def _bb_minimizer(params, x, data=None, **kwargs):
        ...
        redshift = 0.2666
        spectrum = blackbody_spectrum(temperature=temp, scale=scale, extinction_av=extinction_av, extinction_rv=extinction_rv, redshift=redshift)
        ab_model_list = []
        for i in x:
...
def blackbody_spectrum(temperature: float, scale: float, redshift: float=None, extinction_av: float=None, extinction_rv: float=None):
    """ """
    ...
    if extinction_av is not None:
        flux_lambda_reddened = apply(ccm89(np.asarray(wavelengths), extinction_av, extinction_rv), np.asarray(flux_lambda))
        flux_nu_reddened = utilities.flux_lambda_to_nu(flux_lambda_reddened, wavelengths)
        spectrum_reddened = sncosmo_spectral_v13.Spectrum(wave=wavelengths, flux=flux_nu_reddened, unit=utilities.FNU)

    spectrum_unreddened = sncosmo_spectral_v13.Spectrum(wave=wavelengths, flux=flux_nu, unit=utilities.FNU)

    if redshift is not None:
        if extinction_av is not None:
            spectrum_reddened.z = 0
            spectrum_reddened_redshifted = spectrum_reddened.redshifted_to(redshift, cosmo=cosmo)
            return spectrum_reddened_redshifted
        else:
            spectrum_unreddened.z = 0
            spectrum_unreddened_redshifted = spectrum_unreddened.redshifted_to(redshift, cosmo=cosmo)
