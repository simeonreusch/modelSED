    ...
    def fit_global_parameters(self, min_datapoints: int = 4, **kwargs):
        ...
        if self.fittype == "blackbody":
            for iy, y in enumerate(data):
                ...
                # fit_params.add(f"extinction_av_{iy+1}", value=0.1, min=0, max=4)
                fit_params.add(f"extinction_av_{iy+1}", value=0.1, min=0, max=1)
                # fit_params.add(f"extinction_rv_{iy+1}", value=3.1, min=2.5, max=4)
